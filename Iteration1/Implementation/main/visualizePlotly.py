...
def see_map(system, errors):
    ...
    for location in Registry.AVAILABLE_CITIES:
        ...
        y_position = y
        counter += 1
        sensor = system.locations.fromLocation(location)
        if counter == 5:
            x += 10
    ...
    num_columns = len(Registry.AVAILABLE_CITIES) // 5
